    ...
    const safeGasEstimates: (BigNumber | undefined)[] = await Promise.all(
      methodNames.map((methodName, index) =>
        router.estimateGas[methodName](...args)
          .then(res => calculateGasMargin(BigNumber.from(res?.toString())))
          .catch((e) => {
                      ...
                      {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                        <RowBetween style={{ justifyContent: 'flex-end' }}>
                          {oneCurrencyIsETH ? (
                            ...
                            >
                              {TranslateString(1188, 'Receive WPHI')}
                            </StyledInternalLink>
                          ) : oneCurrencyIsWETH ? (
                            <StyledInternalLink
                              to={`/remove/${
                                currencyA && currencyEquals(currencyA, WETH[chainId]) ? 'PHI' : currencyIdA
                              }/${currencyB && currencyEquals(currencyB, WETH[chainId]) ? 'PHI' : currencyIdB}`}
                            >
                              {TranslateString(1190, 'Receive PHI')}
                            </StyledInternalLink>
                          ) : null}
                        </RowBetween>
                      ) : null}
